...
# --- minimal architecture (must match training) ---
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,q,k,v,mask=None):
        B,Tq,_=q.shape; h=self.h; dk=self.d_k
...
class EncoderLayer(nn.Module):
    ...
    def forward(self,x,mask):
        ...
        x=x+self.ff(self.l2(x)); return x
...
class DecoderLayer(nn.Module):
    ...
    def forward(self,y,mem,tgt_mask,mem_mask):
        ...
        y=y+self.ff(self.l3(y)); return y
...
def generate_reply(emotion,situation,customer,mode):
...
if st.button("Generate Reply"):
    reply=generate_reply(emotion,situation,customer,"Greedy")
    st.success(reply)
